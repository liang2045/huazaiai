 ...
 *   2. 监听全局 mouseup: 派发 imade:material-drop CustomEvent 给目标节点
...
const MaterialDragOverlay = () => {
  ...
  useEffect(() => {
    const applyNoNativeDrag = (root: ParentNode = document) => {
      root.querySelectorAll?.('[data-drag-source]').forEach((el) => {
    ...
    const observer = new MutationObserver((records) => {
      for (const record of records) {
        record.addedNodes.forEach((node) => {
          ...
          if (node.matches('[data-drag-source]')) applyNoNativeDrag(node.parentNode || document);
          else applyNoNativeDrag(node);
        });
      }
    });
  ...
  useEffect(() => {
    const onNativeDragStart = (e: DragEvent) => {
      const target = e.target instanceof Element ? e.target.closest('[data-drag-source]') : null;
      if (!target) return;
      e.preventDefault();
      e.stopPropagation();
    };
  ...
  useEffect(() => {
    ...
    const finishDrag = (shouldDrop: boolean, e?: Event) => {
      const cur = useDragMaterialStore.getState();
      if (shouldDrop && cur.dragging && cur.payload && cur.hoverTargetId && cur.hoverAccepts) {
    ...
    const onUp = (e: MouseEvent | PointerEvent) => finishDrag(true, e);

    const onCancel = (e?: Event) => finishDrag(false, e);
