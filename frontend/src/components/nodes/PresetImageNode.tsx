...
/**
 * PresetImageNode - 特殊图像节点(multi-angle-3d / panorama-720 / portrait-preset)
 *
 * 通过 data.preset 区分:
 *   - 'multi-angle-3d' = 3D 多视角(自动循环 4 个视角生成 4 张)
 *   - 'panorama-720'   = 720° 全景(16:9 超宽,固定 prompt 模板)
 *   - 'portrait-preset' = 肖像模板专用(固定风格)
 *
...
const PRESETS: Record<string, PresetMeta> = {
  ...
  'portrait-preset': {
    ...
    promptTemplate: (p) => [
      `polished character portrait, ${p}, soft cartoon style, vibrant colors, expressive eyes, studio lighting, white background, masterpiece`,
    ],
    size: '1024x1024',
  },
};
